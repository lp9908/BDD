<?php
	...
	if($valor == 10){
		...
		try{
			if($estado == 3 && $campo == ""){
				$consulta .= "SELECT * FROM usuario WHERE idUsuario LIKE '%$campo%' OR nombre LIKE '%$campo%' OR tipoUsuario LIKE '%$campo%' OR nombre LIKE '%$campo%' OR ap LIKE '%$campo%' OR am LIKE '%$campo%' OR email LIKE '%$campo%'";
			}else{
				if($estado == 0 && $campo == ""){
				...
				if(($estado == 0 || $estado == 1 || $estado == 3) && $campo != ""){
					...
					switch ($tipoBusqueda) {
						...
						case 6:
							...
							break;
						default:
							$consulta .= " AND	(idUsuario LIKE '%$campo%' OR nombre LIKE '%$campo%' OR tipoUsuario LIKE '%$campo%'	OR nombre LIKE '%$campo%' OR ap LIKE '%$campo%' OR am LIKE '%$campo%' OR email LIKE '%$campo%')";
							break;
					}
				}
			}
